...
for i in range(0,200):
    url = f'https://declarator.org/api/get-year-income/{us_id}/?format=json'
    sql1 = f"INSERT INTO year_income (user_id, year_2018, year_2019, year_2020) VALUES ({us_id}, -1, -1, -1)";
    cursor.execute(sql1)
    con.commit()
    ...
    for i in range(0,parsed_string["count"]):
        if (parsed_string["results"][i]['document__income_year']) == 2018 or (parsed_string["results"][i]['document__income_year']) == 2019 or (parsed_string["results"][i]['document__income_year']) == 2020:
            ...
            a2 = parsed_string["results"][i]["year_income"]
            if a2 == None:
                a2 = -1
            sql = f"UPDATE year_income SET {a1}={a2} WHERE user_id = {us_id}"
            print(sql)
            cursor.execute(sql)
            con.commit()
        else:
